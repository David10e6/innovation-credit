    ...
    <el-card shadow="hover">
      <template #header>
        <div class="card-header">
          <span style="font-size: 16px; font-weight: bold;">院级通知管理</span>
...
// 当前学院
const currentCollege = ref(userStore.userInfo?.college || '未知学院')
const collegeId = ref(userStore.userInfo?.collegeId || '')
...
const rules = {
  ...
  expireTime: [{ required: true, message: '请选择截止时间', trigger: 'change' }]
}
...
const openEditNoticeDialog = (row) => {
  ...
  Object.assign(noticeForm, {
    ...
    expireTime: new Date(row.endTime || row.expireTime)
  })
  fileList.value = []
  dialogVisible.value = true
}
...
const getNotices = async () => {
  try {
    ...
    const response = await noticeAPI.getCollegeNoticeList(params)
    ...
    ElMessage.error(error.response?.data?.msg || '获取通知列表失败')
  } finally {
    loading.value = false
  }
}
...
const submitNotice = async () => {
  ...
  try {
    ...
    if (!collegeId.value) {
      ElMessage.error('用户信息不完整，无法发布通知')
      return
    }
    ...
    if (noticeForm.id) {
      // 编辑操作
      response = await noticeAPI.editCollegeNotice(noticeForm.id, noticeData)
    } else {
      // 新增操作
      response = await noticeAPI.publishCollegeNotice(noticeData)
    }
...
const handleDelete = (id) => {
  ElMessageBox.confirm('是否确认删除该通知？删除后不可恢复', '警告', {
    ...
    try {
      const response = await noticeAPI.deleteCollegeNotice(id)
      if (response.code === 200) {
