...
export const creditApplicationAPI = {
  ...
  // 申请校外活动学分（带名次）
  applyExternalWithRank: (data, rankName) => api.post('/api/credit-application/external-with-rank', data, { params: { rankName } }),
  // 撤回学分申请
  withdraw: (id) => api.post(`/api/credit-application/withdraw/${id}`),
  // 重新提交学分申请
  resubmit: (data) => api.post('/api/credit-application/resubmit', data),
  // 修改驳回的申请（带名次）
  modifyRejectedWithRank: (data, rankName) => api.post('/api/credit-application/modify-with-rank', data, { params: { rankName } }),
  // 删除学分申请
  ...
  // 根据活动类型、级别、获奖名次计算学分
  calculateCredit: (params) => api.get('/api/credit-application/calculate-credit', { params })
}
...
export const counselorAPI = {
  ...
  getApplyList: (collegeId) => {
    ...
    return api.get(`/api/credit-application/college/${realId}`).then(res => res.data);
  },
  // 获取申请详情
  getApplyDetail: (id) => api.get(`/api/credit-application/detail/${id}`).then(res => res.data),
  // 审核申请
  auditApply: (data) => api.post('/api/audit/college', data).then(res => res.data),
  // 获取学生列表
    ...
    return api.get(`/api/student/college/${realId}`)
      ...
      .catch(error => {
        ...
        if (error.response && error.response.status === 403) {
          ...
          return api.get(`/api/student/list/college/${realId}`).then(res => res.data);
        }
        throw error;
      });
  },
  ...
  getCollegeClasses: (collegeId) => {
    ...
    return api.get(`/api/student/college/${realId}/classes`).then(res => res.data);
  },
  // 获取按班级分组的学生列表
  getStudentClassGroups: (collegeId) => {
    ...
    return api.get(`/api/student/college/${realId}/class-group`).then(res => res.data);
  },
  // 获取年级列表
  getGrades: (collegeId) => {
    const realId = typeof collegeId === 'object' ? (collegeId.id || collegeId.collegeId) : collegeId;
    ...
    return api.get(`/api/student/college/${realId}/grades`).then(res => res.data);
  },
  // 获取某年级的班级列表
  getClassesByGrade: (collegeId, grade) => {
    ...
    return api.get(`/api/student/college/${realId}/grades/${grade}/classes`).then(res => res.data);
  },
  // 获取年级+班级对应的学生
  ...
  // 获取辅导员端统计概览数据
  getCounselorStatsOverview: (collegeId) => {
    const realId = typeof collegeId === 'object' ? (collegeId.id || collegeId.collegeId) : collegeId;
    if (typeof realId !== 'number' && typeof realId !== 'string') {
      console.error('学院ID格式错误，应为数字/字符串');
      return Promise.reject(new Error('学院ID参数错误'));
    }
    console.log('获取辅导员端统计概览数据，collegeId:', realId);
    return api.get(`/api/counselor/stats/overview/${realId}`);
  },
  // 获取班级学分统计（用于柱状图）
  getClassCreditStats: (collegeId) => {
    const realId = typeof collegeId === 'object' ? (collegeId.id || collegeId.collegeId) : collegeId;
    if (typeof realId !== 'number' && typeof realId !== 'string') {
      console.error('学院ID格式错误，应为数字/字符串');
      return Promise.reject(new Error('学院ID参数错误'));
    }
    console.log('获取班级学分统计，collegeId:', realId);
    return api.get(`/api/counselor/stats/class-credit/${realId}`);
  },
  // 获取审核效率趋势（用于折线图）
  getAuditEfficiencyTrend: (collegeId) => {
    const realId = typeof collegeId === 'object' ? (collegeId.id || collegeId.collegeId) : collegeId;
    if (typeof realId !== 'number' && typeof realId !== 'string') {
      console.error('学院ID格式错误，应为数字/字符串');
      return Promise.reject(new Error('学院ID参数错误'));
    }
    console.log('获取审核效率趋势，collegeId:', realId);
    return api.get(`/api/counselor/stats/audit-trend/${realId}`);
  },
  // 获取带筛选条件的学生列表
  getStudentsWithFilter: (collegeId, params) => {
    const realId = typeof collegeId === 'object' ? (collegeId.id || collegeId.collegeId) : collegeId;
    if (typeof realId !== 'number' && typeof realId !== 'string') {
      console.error('学院ID格式错误，应为数字/字符串');
      return Promise.reject(new Error('学院ID参数错误'));
    }
    console.log('获取带筛选条件的学生列表，collegeId:', realId, 'params:', params);
    return api.get(`/api/counselor/students/${realId}`, { params }).then(res => res.data);
  }
}
...
export const activityAPI = {
  ...
  // 更新活动参与者信息
  updateParticipant: (data) => api.put(`/api/activity/participants`, data),
  // 带名次添加活动参与者
  addParticipantsWithRank: (activityId, data) => api.post(`/api/activity/participants/${activityId}/with-rank`, data)
}
...
export const adminAPI = {
  ...
  getStudentList: (params) => {
    ...
    return api.get('/api/student/list', { params })
      .then(response => {
        console.log('获取学生列表API响应:', response);
        // 响应拦截器已经处理过，返回的是 { code: 200, data: ... } 或类似结构
        // 需要检查是否有 data 字段
        if (response.data) {
          // 如果 data 本身是数组，直接返回
          if (Array.isArray(response.data)) {
  ...
  // 获取所有学院达标率
  getAllCollegesPassRate: (grade) => {
    console.log('获取所有学院达标率，年级:', grade);
    return api.get('/api/credit/all-colleges-pass-rate', { params: { grade } });
  },
  // 获取某学院的学分种类占比（用于饼图）
  ...
  getAdminStatsOverview: () => {
    console.log('获取管理员端统计概览数据');
    return api.get('/api/admin/stats/overview');
  },
  // 获取所有年级列表
  getGradeList: () => {
    console.log('获取年级列表');
    return api.get('/api/admin/grade/list');
  },
  // 获取所有专业列表
  getMajorList: () => {
    console.log('获取专业列表');
    return api.get('/api/admin/major/list');
  },
  // 获取某专业某年级学分排名TOP10
  getMajorCreditRanking: (grade, major) => {
    console.log('获取专业学分排名，年级:', grade, '专业:', major);
    return api.get('/api/admin/major/credit-ranking', { params: { grade, major } });
  },
  // 根据学院ID获取专业列表
  getMajorsByCollege: (collegeId) => {
    console.log('根据学院ID获取专业列表，collegeId:', collegeId);
    return api.get('/api/major/list', { params: { collegeId } });
  }
}
